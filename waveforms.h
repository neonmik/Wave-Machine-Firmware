...
int get_wavetable (int osc) {

  // read the waveTable, place the pointer at... 
  // waveform == which particular waveform in the array (theres like 16)
  // wave_index == is the current position, this is determined by the frequency and the current increment of that frequency
  // wave_vector == used to move through the waveforms one value at a time... basically creates crazy new waveforms out of less waves.
  //               can be used with either modulation or envelope (both VERY cool)

  return wavetable[(oscillators[osc].waveform+oscillators[osc].wave_index)+oscillators[osc].wave_vector];
}

int get_modulation (int freq) {
  return wavetable[mod_wave+freq];
}