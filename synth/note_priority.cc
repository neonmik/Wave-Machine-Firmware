...
namespace NOTE_PRIORITY {
  ...
  void voice_on(int slot, int note, int velocity) {
    if (note) {
      ...
      MAILBOX::trigger_send(slot, VOICES[slot].note, VOICES[slot].gate);
    } else {
      return;
    }
  }
  void voice_off(int slot, int note, int velocity) {
    ...
    filter_off();
    MIDI::sendNoteOff(VOICES[slot].note, velocity); // Needs reworking for multcore
    MAILBOX::trigger_send(slot, VOICES[slot].note, VOICES[slot].gate); // new voice handling in hardware core
  }
  ...
  void check_release () {
    // loops through the available queue entries till it's empty. this will ned experimentaion.
    uint8_t queue_level = MAILBOX::release_check_queue();
    for (int i = 0; i > queue_level; i++) {
      // receives the slot number thats released from the queue, and then clears the slot on this core.
      uint8_t slot = MAILBOX::release_receive();
      VOICES[slot].clear();
    }
  }
  // transfer from Key/Midi notes to Arp/Note Priority
