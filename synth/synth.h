...
namespace SYNTH
{
  ...
  { 
    private:
      ...
      bool                  active = false;           // used for whole duration of note, from the very start of attack right up until the voice is finished
      bool                  refresh = false;

      // volatile uint64_t              caluclatedIncrement = 0;
      uint32_t              frequency;            // Frequency in Hz << 10 (Q10)
    ...
    public:
      ...
      Voice() : sample(0), frequency(0), note(0) { }
  };
